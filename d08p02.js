...
for (let i = 0; i < entries.length; i++) {
    const [patterns, values] = entries[i].split("|").map((part) =>
        part
            .trim()
            .split(" ")
    ...
    let char_count = { a: 0, b: 0, c: 0, d: 0, e: 0, f: 0, g: 0 };
    let pattern_1;
    for (let j = 0; j < patterns.length; j++) {
        for (let k = 0; k < patterns[j].length; k++) {
            char_count[patterns[j][k]]++;
        }
        if (patterns[j].length === 2) {
            pattern_1 = patterns[j];
        }
    }
    // console.log(char_count);
    // console.log({ pattern_1 });

    let char_4, char_6, char_9;
    for (let char in char_count) {
        if (char_count[char] === 4) {
            char_4 = char;
        } else if (char_count[char] === 6) {
            char_6 = char;
        } else if (char_count[char] === 9) {
            char_9 = char;
        }
    }
    // console.log({ char_4, char_6, char_9 });
    ...
    for (let j = 0; j < patterns.length; j++) {
        ...
        if (pattern.length === 2) {
            ...
            if (pattern.indexOf(char_4) !== -1) {
                res[pattern] = 2;
            } else if (pattern.indexOf(char_6) !== -1) {
                res[pattern] = 5;
            } else {
                res[pattern] = 3;
            }
            ...
            if (pattern.indexOf(char_4) === -1) {
                res[pattern] = 9;
            } else if (pattern.indexOf(pattern_1[0]) !== -1 && pattern.indexOf(pattern_1[1]) !== -1) {
                res[pattern] = 0;
            } else {
                res[pattern] = 6;
            }
